...
<body>
    ...
    <table class="easyui-datagrid" id="userList" title="会员列表" 
	       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/user/list.action',method:'post',pageSize:5,toolbar:toolbar,pageList:[2,5,10]">
	    <thead>
	        <tr>
...
<div id="userAdd" class="easyui-window" title="新增会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/user-add.action'" style="width:800px;height:600px;padding:10px;">
        The window content.
</div>
<div id="userEdit" class="easyui-window" title="修改会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/user-edit.action'" style="width:800px;height:600px;padding:10px;">
	The window content.
</div>
...
var toolbar = [{
    ...
    handler:function(){
    	...
    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的会员吗？',function(r){
    	    if (r){
            	$.post("/user/delete.action",{'ids':ids}, function(data){
        			if(data.status == 200){
